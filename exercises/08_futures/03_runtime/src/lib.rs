    ...
    T: Display + Send + Sync + 'static,
{
    ...
    let handle1 = tokio::spawn(echo_reply(first, Arc::clone(&reply)));
    let handle2 = tokio::spawn(echo_reply(second, reply));

    let _ = tokio::join!(handle1, handle2);
}

async fn echo_reply<T>(listener: TcpListener, reply: Arc<T>)
where
    T: Display + Send + Sync + 'static,
{
    loop {
